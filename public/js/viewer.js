...
var DEFAULT_URL = 'http://dds.crl.edu/';
...
var DEFAULT_ITEM_ID = 0;
var DEFAULT_CURRENT_PAGE = 1;
var DDS_URL = (function getURL() {
...
/* CRL Modification : this is a custom CRL function to generate the url for the PDF stream */
function getFileURL( ) {    
    return DDS_URL + "page/view/"+ PDFView.item +'/'+ PDFView.currentScan;
}
...
var PDFFindBar = {
  ...
  initialize: function(options) {
    ...
    this.findPreviousButton.addEventListener('click',
      function() { console.log('clicked'); self.dispatchEvent('again', true); }
    );
...
var SecondaryToolbar = {
  ...
  initialize: function secondaryToolbarInitialize(options) {
    ...
    this.nextIssue = options.nextIssue;
    this.previousIssue = options.previousIssue;
    ...
    var elements = [
      ...
      { element: this.nextIssue, handler: this.nextIssueClick },
      { element: this.previousIssue, handler: this.previousIssueClick },
  ...
  /* TO DO: CRL Modification for the Next Issue*/
  nextIssueClick: function secondaryToolbarNextIssueClick(evt) {
   // PDFView.page = 1;
   // this.close();
  },
 /* TO DO: CRL Modification for the Previous Issue*/
  previousIssueClick: function secondaryToolbarPreviousIssueClick(evt) {
   // PDFView.page = PDFView.pdfDocument.numPages;
    //this.close();
  },
...
var PDFView = {
  ...
  isViewerEmbedded: true, // Stop the IE and Chrome from Scrolling down
  idleTimeout: null,
  ...
  initialize: function pdfViewInitialize() {
    ...
    SecondaryToolbar.initialize({
      ...
      nextIssue: document.getElementById('next_issue'),
      previousIssue: document.getElementById('previous_issue'),
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
    ...
    /*CRL Modification - Verify if this is single page PDF and if hidden field totalPages has been populated */
    var ddsPagesCount = document.getElementById('totalPages').value;
    
    if(pagesCount == 1 && ddsPagesCount > 1 ) {
   //     pagesCount = ddsPagesCount;
    }
...
var PageView = function pageView(container, id, scale,
  ...
  var ddsViewerPanel = document.getElementById('ddsViewerPanel');
  ...
  this.reset = function pageViewReset(keepAnnotations) {
    ...
    document.getElementById('ddsViewer').style.height = Math.floor(this.viewport.height +300) + 'px';
    ddsViewerPanel.style.height = Math.floor(this.viewport.height +200) + 'px';
    document.getElementById('viewerContainer').style.height = Math.floor(this.viewport.height +100) + 'px';
    document.getElementById('ddsViewerBody').style.height = Math.floor(this.viewport.height +100) + 'px';
...
var doc1; // used to detect DOMContentLoaded event firing twice

function webViewerLoad(evt) {
    var doc = evt.target; // document that triggered "onload" event
    if (doc1 && (doc1 == doc)) return;
...
function webViewerInitialized() {
  ...
  var pathArray = document.location.pathname.split('/');  
    //CRL Modification: set the item id of the Resource
    PDFView.item = parseInt(pathArray[2]);
  
    if( pathArray[3] )
      PDFView.currentScan = parseInt(pathArray[3]);
...
*/
/* CRL Modification : the previous click event will load the previous PDF */
  document.getElementById('previous').addEventListener('click',
    function() {  
      PDFView.currentScan--;
      document.getElementById('pageDDSNumber').value = PDFView.currentScan;  
      PDFView.open(getFileURL(), 0);
      
    });
/* CRL Modification : the next clickj event will load the next PDF */
  ...
  document.getElementById('pageDDSNumber').addEventListener('click',
    function() {
     this.select();
    });
  ...
  document.getElementById('pageDDSNumber').addEventListener('change',
    function() {
      ...
      PDFView.currentScan = (this.value | 0);
  
        
      if (this.value !== (this.value | 0).toString()) {
        this.value = PDFView.currentScan;
      }
      PDFView.open(getFileURL(), 0); // To do: Need some error check for Page range
    });
...
window.addEventListener('pagechange', function pagechange(evt) {
  ...
  var page = parseInt(document.getElementById('pageDDSNumber').value);
  var totalfiles = parseInt(document.getElementById('totalPages').value);
  if( page > totalfiles)
      page = totalfiles;
  document.getElementById('previous').disabled = (page <= 1);
  document.getElementById('next').disabled = (page >= totalfiles);
}, true);
