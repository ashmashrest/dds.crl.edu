...
TypeAheadControl.prototype.draw = function() {
    ...
    this.dropDown.onmouseover = function(b) {
        if (!b) {
            b = window.event
        }
        var c;
...
TypeAheadControl.prototype.populateDropDown = function(c) {
    ...
    while (b < c.length) {
        ...
        this.dropDown.appendChild(f);
        b++
    }
    var d = this.getXY();
    this.dropDown.style.left = d[0] + "px";
    this.dropDown.style.top = d[1] + "px";
    this.showDrop()
};
...
function IdPSelectUI() {
    ...
    var am = [];
    var at = "entityID";
    this.draw = function(az) {
        if (!j(az)) {
            return
        }
        aw = document.getElementById(az.insertAtDiv);
        if (!aw) {
            H(w("fatal.divMissing"));
            return
        }
        if (!d(az.dataSource)) {
            return
    ...
    var j = function(ay) {
        var aI;
        y = ay.preferredIdP;
        U = ay.maxPreferredIdPs;
        J = ay.helpURL;
        f = ay.ie6Hack;
        D = ay.samlIdPCookieTTL;
        Y = ay.alwaysShow;
        aj = ay.maxResults;
        ad = ay.ignoreKeywords;
        z = ay.defaultLogo;
        k = ay.defaultLogoWidth;
        u = ay.defaultLogoHeight;
        h = ay.minWidth;
        ai = ay.minHeight;
        e = ay.maxWidth;
        p = ay.maxHeight;
        R = ay.bestRatio;
        G = ay.maxIdPCharsButton;
        af = ay.maxIdPCharsDropDown;
        ...
        aa = ay.defaultLanguage;
        if (typeof ay.langBundles[K] != "undefined") {
            Q = ay.langBundles[K]
        } else {
        ...
        c = ay.langBundles[ay.defaultLanguage];
        if (!c) {
            H("No languages work");
            return false
        }
        if (!Q) {
            o("No language support for " + K);
            Q = c
        }
        if (ay.testGUI) {
            return true
        }
        var aE = "urn:oasis:names:tc:SAML:profiles:SSO:idpdiscovery-protocol:single";
        var aA;
        var az = false;
        var aG;
        var aF;
        var aC = window;
        while (null !== aC.parent && aC !== aC.parent) {
            aC = aC.parent
        }
        var aB = aC.location;
        var aD = aB.search;
        if (null == aD || 0 == aD.length || aD.charAt(0) != "?") {
            ...
            aI = ay.myEntityID;
            x = ay.defaultReturn;
            if (null != ay.defaultReturnIDParam) {
            ...
            H(w("fatal.wrongProtocol"));
            return false
        }
        if (ay.myEntityID !== null && ay.myEntityID != aI) {
            H(w("fatal.wrongEntityId") + '"' + aI + '" != "' + ay.myEntityID + '"');
            return false
        }
        if (null === x || x.length === 0) {
            H(w("fatal.noReturnURL"));
            return false
        }
        if (!ab(x)) {
            H(w("fatal.badProtocol"));
            return false
        }
        if (az) {
            var aJ = b();
            if (aJ.length == 0) {
                ...
                var aH = at + "=" + encodeURIComponent(aJ[0]);
                if (x.indexOf("?") == -1) {
                    aH = "?" + aH
                } else {
                    aH = "&" + aH
    ...
    var ab = function(az) {
        ...
        var ay = "://";
        var aA = az.indexOf(ay);
        if (aA < 0) {
            return false
        }
        az = az.substring(0, aA);
        if (az == "http" || az == "https") {
            return true
        }
        return false
    ...
    var d = function(aB) {
        ...
        if (null == aA) {
            try {
                aA = new ActiveXObject("MSXML2.XMLHTTP.3.0")
            } catch (az) {}
        }
        if (null == aA) {
            H(w("fatal.noXMLHttpRequest"));
            return false
        }
        if (av()) {
            aB += "?random=" + (Math.random() * 1000000)
        }
        aA.open("GET", aB, false);
        if (typeof aA.overrideMimeType == "function") {
            aA.overrideMimeType("application/json")
        }
        aA.send(null);
        if (aA.status == 200) {
            var ay = aA.responseText;
            if (ay === null) {
                H(w("fatal.noData"));
                return false
            }
    ...
    var A = function(aD) {
            ...
            for (aF in aD.Logos) {
                if (aD.Logos[aF].lang == aG && aD.Logos[aF].width != null && aD.Logos[aF].width >= h && aD.Logos[aF].height != null && aD.Logos[aF].height >= ai) {
                    if (aE === null) {
                        ...
                        me = Math.abs(R - Math.log(aD.Logos[aF].width / aD.Logos[aF].height));
                        him = Math.abs(R - Math.log(aE.width / aE.height));
                        if (him > me) {
                            aE = aD.Logos[aF]
                        }
                    }
        ...
        aA = aC(K);
        if (null === aA && typeof an != "undefined") {
            aA = aC(an)
        }
        if (null === aA) {
            aA = aC(null)
        ...
        az.setAttribute("width", ay);
        az.setAttribute("height", aB);
        return az
    };
    var S = function() {
        var az = ac("IdPSelector");
        var ay;
        ay = ao(az);
        l(az, ay);
        O(az, ay);
    ...
    var E = function(az, aG) {
        var ay = ac(undefined, "PreferredIdPButton");
        var aF = document.createElement("a");
        var aE = at + "=" + encodeURIComponent(v(az));
        var aA = x;
        var aC = A(az);
        if (aA.indexOf("?") == -1) {
            aE = "?" + aE
        ...
        var aH = ac(undefined, "PreferredIdPImg");
        aH.appendChild(aC);
        aF.appendChild(aH);
        var aD = ac(undefined, "TextDiv");
        var aB = aq(az);
        ay.title = aB;
        if (aB.length > G) {
            aB = aB.substring(0, G) + "..."
        }
        aD.appendChild(document.createTextNode(aB));
        aF.appendChild(aD);
        ay.appendChild(aF);
        return ay
    };
    var ar = function(ay, aB) {
        var aA = ac(undefined, "TextDiv");
        var az = document.createTextNode(w(aB));
        aA.appendChild(az);
        ay.appendChild(aA)
    };
    var a = function(ay, aA) {
        if (null === aA || 0 === aA.length || "-" == aA.value) {
            return
        }
        var az = 0;
        while (az < ay.options.length) {
            if (ay.options[az].value == aA) {
                ay.options[az].selected = true;
                break
            }
            az++
        }
    ...
    var ao = function(aC) {
        ...
        var aA = ac("PreferredIdPTile");
        ar(aA, "idpPreferred.label");
        for (var az = 0; az < U && az < aB.length; az++) {
            if (aB[az]) {
                var ay = E(aB[az], az);
                aA.appendChild(ay)
            }
    ...
    var V = function() {
        var az = document.createElement("form");
        L.appendChild(az);
        az.action = P;
        az.method = "GET";
        az.setAttribute("autocomplete", "OFF");
        var ay = 0;
    ...
    var l = function(aE, az) {
        L = ac("IdPEntryTile");
        if (az) {
            ar(L, "idpEntry.label")
        } else {
            ar(L, "idpEntry.NoPreferred.label")
        }
        var aC = V();
        ...
        aC.onsubmit = function() {
            ...
            aB.value = aD.textValue;
            au(aD.value);
            return true
        };
        ae = new TypeAheadControl(n, aB, aD, aA, af, aq, v, X, f, Y, aj, B);
        var ay = document.createElement("a");
        ay.appendChild(document.createTextNode(w("idpList.showList")));
        ay.href = "#";
        Z(ay, "DropDownToggle");
        ay.onclick = function() {
    ...
    var O = function(aF, aA) {
        g = ac("IdPListTile");
        g.style.display = "none";
        if (aA) {
            ar(g, "idpList.label")
        } else {
            ar(g, "idpList.NoPreferred.label")
        }
        ak = document.createElement("select");
        i(ak, "Selector");
        ak.name = at;
        g.appendChild(ak);
        var az = m("-", w("idpList.defaultOptionLabel"));
        az.selected = true;
        ak.appendChild(az);
        var aE;
        ...
        aD.onsubmit = function() {
            ...
            au(ak.options[ak.selectedIndex].value);
            return true
        };
        var aB = r("List");
        M = aB;
        ...
        g.appendChild(aD);
        var ay = document.createElement("a");
        ay.appendChild(document.createTextNode(w("idpList.showSearch")));
        ay.href = "#";
        Z(ay, "DropDownToggle");
        ay.onclick = function() {
    ...
    var r = function(az) {
        ...
        ay.setAttribute("type", "submit");
        ay.value = w("submitButton.label");
        i(ay, az + "Button");
        return ay
    };
    var t = function(az) {
    ...
    var m = function(az, aA) {
        var ay = document.createElement("option");
        ay.value = az;
        if (aA.length > af) {
            aA = aA.substring(0, af)
        }
        ay.appendChild(document.createTextNode(aA));
        return ay
    };
    var i = function(az, ay) {
        az.id = ax + ay
    };
    var Z = function(az, ay) {
        az.setAttribute("class", W + ay)
    };
    var ap = function(ay) {
        return document.getElementById(ax + ay)
    };
    var au = function(ay) {
        C(ay);
        ag(I)
    ...
    var X = function(aA) {
        ...
        for (ay = 0; ay < aA.Logos.length; ay++) {
            var az = aA.Logos[ay];
            if (az.height == "16" && az.width == "16") {
                if (null == az.lang || K == az.lang || (typeof an != "undefined" && an == az.lang) || aa == az.lang) {
                    return az.value
                }
            }
        }
    ...
    var aq = function(az) {
        var ay = al(az.DisplayNames);
        if (null !== ay) {
            return ay
        }
        o("No Name entry in any language for " + v(az));
        return v(az)
    };
    var B = function(az) {
        if (ad || null == az.Keywords) {
            return null
        }
        var ay = al(az.Keywords);
        return ay
    };
    var al = function(ay) {
        var az;
    ...
    var F = function() {
        ...
        var ay;
        if (null != y) {
            for (az = 0; az < y.length && az < U - 1; az++) {
                aB[az] = T(y[az]);
                aA++
            }
        }
        I = b();
        for (az = aA, ay = 0; az < I.length && az < U; az++, ay++) {
            aB.push(T(I[ay]))
        }
        return aB
    };
    ...
    var b = function() {
        ...
        for (aD = 0; aD < aG.length; aD++) {
            var ay = aG[aD];
            var aC = ay.indexOf("=");
            var aF = ay.substring(0, aC);
            var aA = ay.substring(aC + 1);
            if ("_saml_idp" == aF.replace(/^\s+|\s+$/g, "")) {
                aA = aA.replace(/^\s+|\s+$/g, "").split("+");
                for (aB = 0; aB < aA.length; aB++) {
                    if (0 === aA[aB].length) {
                        continue
                    }
                    var az = ah(decodeURIComponent(aA[aB]));
                    if (az.length > 0) {
                        aE.push(az)
                    }
                }
    ...
    var ag = function(aD) {
        ...
        for (var aB = 0; aB < aC; aB++) {
            if (aD[aB].length > 0) {
                aA.push(encodeURIComponent(s(aD[aB])))
            }
        }
        var ay = null;
        if (D) {
            var az = new Date();
            cookieTTL = D * 24 * 60 * 60 * 1000;
            ay = new Date(az.getTime() + cookieTTL)
        }
        document.cookie = "_saml_idp=" + aA.join("+") + "; path = /" + ((ay === null) ? "" : "; expires=" + ay.toUTCString())
    };
    var s = function(aH) {
        var ay = "",
            aC, aA, az, aG, aF, aE, aD;
        for (var aB = 0; aB < aH.length;) {
            aC = aH.charCodeAt(aB++);
            aA = aH.charCodeAt(aB++);
            az = aH.charCodeAt(aB++);
            aG = aC >> 2;
            ...
            ay += N.charAt(aG) + N.charAt(aF) + N.charAt(aE) + N.charAt(aD)
        }
        return ay
    };
    var ah = function(aB) {
        var az = "",
            aI, aG, aE, aH, aF, aD, aC;
        var aA = 0;
        var ay = /[^A-Za-z0-9\+\/\=]/g;
        aB = aB.replace(/[^A-Za-z0-9\+\/\=]/g, "");
        do {
            aH = N.indexOf(aB.charAt(aA++));
            aF = N.indexOf(aB.charAt(aA++));
            aD = N.indexOf(aB.charAt(aA++));
            aC = N.indexOf(aB.charAt(aA++));
            aI = (aH << 2) | (aF >> 4);
            aG = ((aF & 15) << 4) | (aD >> 2);
            aE = ((aD & 3) << 6) | aC;
            az = az + String.fromCharCode(aI);
            if (aD != 64) {
                az = az + String.fromCharCode(aG)
            }
            if (aC != 64) {
                az = az + String.fromCharCode(aE)
            }
            aI = aG = aE = "";
            aH = aF = aD = aC = ""
        } while (aA < aB.length);
    ...
    var H = function(az) {
        alert("FATAL - DISCO UI:" + az);
        var ay = document.createTextNode(az);
        aw.appendChild(ay)
    };
    var o = function() {}
